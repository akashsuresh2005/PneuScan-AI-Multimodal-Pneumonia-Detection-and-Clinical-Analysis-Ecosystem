...
const ResultCard = ({ result, originalFile }) => {
  ...
  return (
    <div className="bg-white rounded-[24px] sm:rounded-[40px] shadow-2xl border border-slate-100 overflow-hidden animate-in zoom-in duration-500">
      
      {/* 1. Side-by-Side Imaging Suite */}
      <div className="grid grid-cols-1 sm:grid-cols-2 bg-slate-900 h-[440px] sm:h-[320px]">
        <div className="relative group h-[220px] sm:h-full">
          {originalFile && (
            <img src={URL.createObjectURL(originalFile)} alt="Original" className="w-full h-full object-cover opacity-80" />
          )}
          <span className="absolute bottom-4 left-4 bg-black/50 text-white text-[10px] px-3 py-1 rounded-full font-bold uppercase backdrop-blur-md">Original X-Ray</span>
        </div>
        <div className="relative group border-t sm:border-t-0 sm:border-l border-white/10 h-[220px] sm:h-full">
          <img src={`https://pneuscan-ai-multimodal-pneumonia-jhqi.onrender.com/uploads/${result.heatmapPath}`} alt="Heatmap" className="w-full h-full object-cover" />
          <span className="absolute bottom-4 left-4 bg-blue-600/80 text-white text-[10px] px-3 py-1 rounded-full font-bold uppercase backdrop-blur-md">AI Heatmap Visualization</span>
        </div>
      </div>
      ...
      {/* LOW CONFIDENCE SYSTEM INTERCEPT ALERT CARD */}
      {isLowConfidence && (
        <div className="mx-4 sm:mx-8 mt-6 p-4 sm:p-5 bg-amber-50 border border-amber-200 rounded-2xl sm:rounded-3xl flex items-start gap-3 sm:gap-4 animate-in fade-in duration-300">
          <div className="p-2 bg-amber-500 text-white rounded-xl font-black text-sm shrink-0">⚠️</div>
          <div>
            <h4 className="text-sm font-black text-amber-800 uppercase tracking-tight">Low-Confidence System Warning</h4>
            <p className="text-xs text-amber-700/90 font-semibold mt-1 leading-relaxed">
      ...
      {/* 2. Diagnosis Status Badge */}
      <div className={`relative flex justify-center mt-6 z-10 px-4 sm:px-8 transition-all duration-300 ${isLowConfidence ? 'opacity-30 grayscale pointer-events-none' : ''}`}>
        <div className={`flex items-center gap-4 sm:gap-5 px-5 sm:px-8 py-4 sm:py-5 rounded-2xl sm:rounded-3xl shadow-xl border-b-4 w-full ${
          isNormal 
            ? 'bg-emerald-600 text-white border-emerald-800' 
            : isCovid
            ? 'bg-purple-700 text-white border-purple-900 animate-pulse'
          ...
          {isNormal ? <CheckCircle size={28} className="sm:w-8 sm:h-8 shrink-0" /> : <ShieldAlert size={28} className={`sm:w-8 sm:h-8 shrink-0 ${isCovid ? "" : "animate-pulse"}`} />}
          <div className="flex flex-col min-w-0">
            <span className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.2em] sm:tracking-[0.25em] text-white/80 leading-none mb-1.5">
              Radiological Status
            </span>
            <span className="text-lg sm:text-2xl font-black tracking-tight uppercase leading-tight whitespace-normal break-words">
              {predictionStr}
            </span>
          </div>
        </div>
      ...
      <div className="p-4 sm:p-8 pt-6 space-y-6 sm:space-y-8">
        
        {/* 3. Advanced Diagnostic Metrics */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
            <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl sm:rounded-3xl flex items-center gap-4">
                <div className="bg-blue-100 p-3 rounded-2xl text-blue-600 shrink-0"><MapPin size={20}/></div>
                <div className="min-w-0">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Anatomic Focus</p>
                    <p className="text-sm font-bold text-slate-700 break-words">{result.localization || "Analyzing Zones..."}</p>
                </div>
            </div>

            <div className="bg-slate-50 border border-slate-100 p-4 rounded-2xl sm:rounded-3xl flex items-center gap-4">
                <div className="bg-orange-100 p-3 rounded-2xl text-orange-600 shrink-0"><Activity size={20}/></div>
                <div className="min-w-0">
                    <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Severity Tier</p>
                    <p className="text-sm font-bold text-slate-700 break-words">{result.severity || "Calculating..."}</p>
                </div>
            </div>
        </div>

        {/* Probability Assessment Section */}
        <div className="bg-slate-50/80 p-4 sm:p-6 rounded-[22px] sm:rounded-[30px] border border-slate-100">
          <div className="flex flex-wrap justify-between items-end gap-3 mb-4">
            <div>
              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Neural Core Probability</p>
              <h3 className="text-3xl sm:text-5xl font-black text-slate-900 tracking-tighter leading-none">{confidence}%</h3>
            </div>
            <div className="flex flex-col items-end">
                <span className="text-[9px] font-black text-slate-500 uppercase px-2 py-1 bg-white rounded-md border border-slate-200 mb-1">Accuracy Grade</span>
                <span className="text-[10px] font-bold text-slate-400">Model v1.2</span>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
          <div className="p-5 sm:p-6 bg-blue-50/40 rounded-[22px] sm:rounded-[30px] border border-blue-100/50">
            <h4 className="flex items-center gap-2 font-black text-blue-600 text-xs mb-4 uppercase tracking-widest leading-none"><ListChecks size={16}/> Clinical Guidelines</h4>
            <ul className="space-y-3 text-[11px] text-blue-900/80 font-bold">
              {!isNormal ? (
                <>
          ...
          <div className="p-5 sm:p-6 bg-slate-50/50 rounded-[22px] sm:rounded-[30px] border border-slate-200/50 flex flex-col justify-center items-center text-center">
            <div className="p-4 bg-white rounded-2xl shadow-sm mb-3">
              <Award className="text-slate-300" size={32}/>
            </div>
            <p className="text-[10px] font-black text-slate-800 uppercase tracking-widest leading-none">Verified Diagnostic</p>
        ...
        <button onClick={() => generateProfessionalPDFReport(result)} className="w-full bg-slate-900 text-white py-4 sm:py-5 rounded-[18px] sm:rounded-[25px] font-black flex items-center justify-center gap-3 hover:bg-blue-600 transition-all shadow-xl shadow-slate-200 text-sm sm:text-base">
          <Download size={18} className="sm:w-5 sm:h-5"/> <span className="text-center">DOWNLOAD CLINICAL RADIOLOGY REPORT</span>
        </button>
      </div>
    </div>
  );
